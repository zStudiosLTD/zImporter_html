...
export class ZSpine extends ZContainer {
     ...
     */
    async load(basePath, jsonPath, atlasPath, pngPaths, skin = 'default', width = 512, height = 512) {
        ...
        try {
            ...
            // Load textures.
            const textures = {};
            await Promise.all(pngPaths.map(p => new Promise((resolve) => {
                const img = new Image();
            ...
            // Build TextureAtlas.
            const atlasLoader = new spine.TextureAtlas(atlasText, (path) => {
                const img = textures[path];
                if (!img) {
                    console.warn('[ZSpine] missing texture:', path);
                    return null;
                }
